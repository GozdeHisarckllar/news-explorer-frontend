    ...
    <li className="news-card">
      <article className="news-card__article">
        ...
        <Link className="news-card__description" to={{pathname: newsCard.link }} target='_blank'>
          <time dateTime={newsCard.date} className="news-card__date">
            {new Date(newsCard.date).toLocaleString('en-US', {day:'numeric', year:'numeric',month:'long'})}
          </time>
      ...
      </article>
      {
        isHomeRendered ?
        <>
          <button className={`news-card__btn news-card__btn_type_save ${isSaved ? 'news-card__btn_saved':''}`} 
            onClick={handleSave} disabled={loggedIn ? false : true} type="button" aria-label="create a bookmark">
          </button>
